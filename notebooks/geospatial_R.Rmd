...
Alternatively, we can narrow down to bus stops that are within Davidson County.
CRS = Coordinate Ref System


```{r}
bus_zips_alt <- st_join(bus_stops_geo, zipcodes, join = st_within, left = FALSE)
bus_zips_alt
```  
...
zipcodes |> 
  ggplot() +
  geom_sf() +
  geom_sf(data = bus_zips_alt, size = 0.1)
```
